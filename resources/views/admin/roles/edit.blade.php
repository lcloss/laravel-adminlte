                ...
                <div class="card card-default">
                    ...
                    <div class="card-body">
                        <form action="{{ isset( $role ) ? route('admin.roles.update', $role) : route('admin.roles.store') }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Name</label>
                                        <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Role Name" value="{{ old('role', $role->name ?? '') }}">
                                        @error('name')
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button type="submit" class="btn btn-primary px-5">{{ isset( $role ) ? 'Update' : 'Create' }}</button>
                                    <a href="{{ route('admin.roles.index') }}" role="button" class="btn btn-danger px-5">List</a>
                                </div>
                            </div>
                        </form>
                    </div>
